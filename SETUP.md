...
proxy (Caddy or nginx) to be reachable on port 80/443. Configure authentication for `/admin`
and `/api/admin/*` in your reverse proxy – see the Caddyfile example below.
...
Protect `/admin` and `/api/admin/*` in your reverse proxy (see Caddyfile example in the Architecture section).
