...
def normalize_title(title):
    ...
    if not title:
        return title
    # Normalise mathematical styled chars (𝘼𝙈𝘼𝙑𝙄𝙄 → AMAVII) then strip remaining
    # non-Latin characters (Japanese, emoji, etc.) — Inter font lacks glyphs for them.
    # Keeps: Basic Latin + Latin Extended (U+0020–024F), common punctuation, euro sign.
    title = unicodedata.normalize('NFKC', title)
    title = re.sub(r'[^ -ɏ‐-⁄⁰-₟€]', '', title).strip()
    if not title:
        return title
...
def normalize_venue(venue):
    """
    Fixes venue names written entirely in ALL CAPS. Neither events nor venues
    are allowed to shout: some sources publish venue names in caps, and they
    would otherwise stand out in the listing.

    Unlike normalize_title, nothing else is touched. Correctly capitalised and
    lowercase names are returned exactly as they are, and names containing
    digits fall outside _is_allcaps and pass through untouched.
    """
    if not venue or not _is_allcaps(venue):
        return venue
    function_words = _SV_FUNCTION_WORDS if _is_swedish(venue) else _EN_FUNCTION_WORDS
    return _title_case(venue, function_words)


def apply_replacements(text, rules):
    """
    Applies (pattern, replacement) regex rules in order.

    Used for organiser names sources prepend to titles, and for the street
    addresses and directions they append to venue names.
    """
    if not text:
        return text
...
from config import (CATEGORY_KEYWORDS, VENUE_DEFAULTS, TITLE_BLACKLIST,
                    TITLE_REPLACEMENTS, VENUE_REPLACEMENTS)

# Kategorier som tillhör barn.kristall.info – scraper-tilldelad barn-kategori
# har högre prioritet än keyword-matches för att undvika att barn-venues
# (Kulturhuset Bergsjön, biblioteksfilialer m.fl.) hamnar på vanliga kristall.
_BARN_CATS = frozenset({
...
# Title substrings filtered at the city-config level – see config.TITLE_BLACKLIST
_SKIP_TITLE_SUBSTRINGS = ()  # kept for backwards compat; use config.TITLE_BLACKLIST instead
...
def save_events(events, source='unknown', keep_category=False):
    """
    ...
    Args:
        ...
        keep_category: Use the scraper's category as-is, skipping keyword and
            venue matching (festival views where category is the programme type)
    ...
    try:
        ...
        insert_query = """
            INSERT INTO events (title, date, end_date, venue, category, link, source, description, manual)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, FALSE)
            ON DUPLICATE KEY UPDATE
                title = IF(manual = FALSE, VALUES(title), title),
                end_date = IF(manual = FALSE, VALUES(end_date), end_date),
        ...
        for event in events:
            try:
                # Fix ALL CAPS venue names before they are used for lookups,
                # manual-event deduplication and insertion.
                if event.get('venue'):
                    event['venue'] = normalize_venue(
                        apply_replacements(event['venue'], VENUE_REPLACEMENTS))
                ...
                # Stam, inte hel form: 2Lång skriver "Privat evenemang/abonnerat"
                # och slank förbi när villkoret bara täckte 'abonnerad'.
                if 'abonner' in combined or 'abboner' in combined:
                ...
                if any(bl in raw_title.lower() for bl in TITLE_BLACKLIST):
                    print(f"⏭️  Skippar (blacklist): {raw_title}")
                    _run_stats['skipped_cancelled'] += 1
                    continue
                ...
                clean_title = apply_replacements(
                    normalize_title(event['title']), TITLE_REPLACEMENTS)
                ...
                scraper_cat = event.get('category')
                if keep_category and scraper_cat:
                    category = scraper_cat
                elif keyword_cat and keyword_cat in _BARN_CATS:
                    # Barn-keyword-match specificerar kategorin ytterligare (t.ex. sagostund→Läs & Lyssna)
                    category = keyword_cat
                elif scraper_cat and scraper_cat in _BARN_CATS:
                    # Scraper satte barn-kategori – blockerar adult keyword-match
                    # (vernissage→Konst, konsert→Musik når aldrig hit)
                    category = scraper_cat
                elif keyword_cat:
                    category = keyword_cat
                elif scraper_cat and scraper_cat not in ('Övrigt',):
                    category = scraper_cat
                else:
