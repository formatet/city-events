...
// /bokmassan = Bokmässans program, aldrig blandat med stadens flöde. Läses ur
// URL:en, aldrig ur localStorage. Deklareras här och inte i app.js eftersom
// render.js delas med den publika mallen; live/app.js sätter bokMode och väljer
// datakälla (se app.py: bokmassan_page, /api/bokmassan/events).
let bokMode = 'off';
let bokOnly = window.location.pathname.replace(/\/+$/, '') === '/bokmassan';
let visibleCount = 0;
let filteredEvents = [];
...
function renderRows() {
  ...
  // Filter/omrendrering byter ut innehållet under fötterna på användaren – utan
  // detta blir man kvar på samma pixel-offset (fel datum, eller klampad till
  // botten/citatet om nya listan är kortare). Återställ alltid till toppen.
  window.scrollTo(0, 0);
  // Mässvyn döljer kolumn fyra (CSS body.bok-view): Var säger redan scen eller
  // monter, och biljettkraven står som suffix i titeln.
  document.body.classList.toggle('bok-view', bokOnly);
  ...
  // Festivallänkarna delar mittenplatsen i rubriken. (Bokmässan) byter hela
  // listan mellan stad och mässa (toggleBokView i live/app.js) – vyerna blandas
  // aldrig, till skillnad från WoW. href finns kvar för öppna-i-ny-flik.
  const showWow = (wowMode === 'on' || wowOnly) && !bokOnly;
  const showBok = (bokMode === 'on' || bokOnly) && !wowOnly;
  if (showWow || showBok) {
    hdrEvent.innerHTML =
      `<span class="event-header"><span>Vad</span><span>` +
      (showWow ? `<a href="#" class="wow-link${(currentFilter.wow || wowOnly) ? ' active' : ''}" onclick="toggleWowFilter(event)">(WoW)</a>` : '') +
      (showWow && showBok ? ' ' : '') +
      (showBok ? `<a href="${bokOnly ? '/' : '/bokmassan'}" class="wow-link${bokOnly ? ' active' : ''}" onclick="toggleBokView(event)">(bokmässan)</a>` : '') +
      `</span><span></span></span>`;
  } else {
  ...
  const _catHdr = window._CATEGORY_HEADER || {desktop: 'Konstform', mobile: 'Konst'};
  document.getElementById("header-category").innerHTML =
    `<span class="has-icon">${isMobile ? _catHdr.mobile : _catHdr.desktop}${getKristallIcon(currentFilter.category !== null)}</span>`;
...
const FEED_END_QUOTES = [
  ...
  { t: '"Denna dagen ett liv."',
    a: "Farbror Melker, i Astrid Lindgrens Vi på Saltkråkan (1964)." },
...
function renderChunk() {
  ...
  slice.forEach(event => {
    ...
    const endTimeStr = (window._SHOW_END_TIME && event.end_date)
      ? '–' + formatDate(event.end_date).time
      : '';
    const timeStr = `, ${time}${endTimeStr}`;

    const crystalColor = event.crystal_color || (event.featured ? (window._FEATURED_CRYSTAL || '#bef2ff') : null);
    const row = document.createElement("tr");
    if (event.highlight) row.classList.add(`variant-${event.highlight}`);
    row.innerHTML = `
      <td>
        <span>
          <a href="#" class="filter-date" data-date="${dateKey}">${label}</a>${timeStr}
        </span>
      </td>
      <td>
