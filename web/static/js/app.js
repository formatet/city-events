...
let events = [];
let wowMode = 'off';
// /wow = festival-only view. Read from the URL, never from localStorage.
// Requires festival mode to be enabled in /admin – if it is off, Flask
// redirects /wow to / before we get here (see app.py: wow_page).
let wowOnly = window.location.pathname.replace(/\/+$/, '') === '/wow';
let currentFilter = {
  date: null,
  venue: null,
  category: null,
...
function eventInit() {
  ...
  Promise.all([
    ...
    if (wowOnly) currentFilter.wow = true;
    ...
    document.getElementById('ntfy-report').addEventListener('click', e => {
      ...
      fetch('https://ntfy.sh/your-ntfy-topic-here', {
        method: 'POST',
