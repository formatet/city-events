...
CORS(app, resources={r"/api/events": {"origins": "*"}, r"/data/*": {"origins": "*"}, r"/api/barn/events": {"origins": "*"}, r"/api/klubb/events": {"origins": "*"}})
...
_barn_cache   = {"data": None, "ts": 0.0}
_klubb_cache  = {"data": None, "ts": 0.0}
_bok_cache    = {"data": None, "ts": 0.0}
...
def fmt_events(events):
    for event in events:
        ...
        elif 'end_date' not in event:
            event['end_date'] = None
    return events

@app.route('/')
def index():
    if request.host.startswith('barn.'):
        return send_from_directory('static/barn', 'index.html')
    if request.host.startswith('klubb.'):
        return send_from_directory('static/klubb', 'index.html')
    html = _index_html_with_jsonld()
    if html is None:
...
def get_events():
    ...
    try:
        ...
        cursor.execute(f"""
            ...
            FROM events
            WHERE date >= NOW()
              AND category NOT IN ('{barn_cats}')
              AND COALESCE(source, '') <> '{BOKMASSAN_SOURCE}'
            ORDER BY date ASC
        """)
...
# Bokmässans program (~4 400 punkter) visas bara i festivalvyn /bokmassan och
# hålls utanför stadens flöde och JSON-LD – annars laddar varje besökare
# trefalt data för en vy de flesta aldrig öppnar. Se /api/bokmassan/events.
BOKMASSAN_SOURCE = 'Bokmässan'
...
# --- schema.org Event i startsidans HTML (osynligt, för sökmotorer) ---
# Startsidan renderas klientsidan, så utan detta ser Google inga events alls.
# Egen query i stället för _events_cache: den håller ~2300 events, JSON-LD
# ska bara ha de närmaste dagarna.
_seo_cache = {"html": None, "ts": 0.0}
SEO_EVENT_DAYS = 7
SEO_EVENT_LIMIT = 60

def _fetch_seo_events():
    conn = get_db_connection()
    if not conn:
        return []
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute(f"""
            SELECT title, date, venue, link
            FROM events
            WHERE date >= NOW()
              AND date < NOW() + INTERVAL {SEO_EVENT_DAYS} DAY
              AND category NOT IN ('{BARN_CATS_SQL}')
              AND COALESCE(source, '') <> '{BOKMASSAN_SOURCE}'
            ORDER BY date ASC
            LIMIT {SEO_EVENT_LIMIT}
        """)
        ...
        print(f"SEO event query error: {err}")
        return []

def _build_event_jsonld(events):
    items = []
    for i, e in enumerate(events, start=1):
        ...
        items.append({
            ...
            "item": {
                "@type": "Event",
                "name": e['title'],
                "startDate": date.isoformat() if isinstance(date, datetime) else str(date),
                ...
                "url": e['link'] or "https://kristall.info/",
            },
        })
    payload = {
        "@context": "https://schema.org",
        "@type": "ItemList",
        "name": "Evenemang i Göteborg",
...
def _index_html_with_jsonld():
    """Startsidans HTML med Event-JSON-LD inlagt före </head>.
    Returnerar None om något går fel – anroparen faller tillbaka på filen."""
    now = time.time()
    if _seo_cache["html"] is not None and now - _seo_cache["ts"] < CACHE_TTL:
        return _seo_cache["html"]
    try:
        path = os.path.join(app.static_folder, 'index.html')
        with open(path, encoding='utf-8') as fh:
            html = fh.read()
        events = _fetch_seo_events()
        if not events:
            return None
        html = html.replace('</head>', _build_event_jsonld(events) + '</head>', 1)
        _seo_cache["html"] = html
        _seo_cache["ts"] = now
        return html
    except (OSError, ValueError, KeyError) as err:
        print(f"JSON-LD injection failed: {err}")
        return None

@app.route('/api/barn/events')
def get_barn_events():
    now = time.time()
    if _barn_cache["data"] is not None and now - _barn_cache["ts"] < CACHE_TTL:
        resp = jsonify(_barn_cache["data"])
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp

    conn = get_db_connection()
    if not conn:
        if _barn_cache["data"] is not None:
            resp = jsonify(_barn_cache["data"])
            resp.headers['Cache-Control'] = 'public, max-age=60'
            return resp
        return jsonify({"error": "Database connection failed"}), 500
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute(f"""
            SELECT id, title, date, end_date, venue, category, link, featured, highlight, crystal_color
            FROM events
            WHERE date >= NOW()
              AND category IN ('{BARN_CATS_SQL}')
            ORDER BY date ASC
        """)
        events = fmt_events(cursor.fetchall())
        cursor.close()
        conn.close()
        _barn_cache["data"] = events
        _barn_cache["ts"] = now
        resp = jsonify(events)
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp
    except mysql.connector.Error as err:
        print(f"Database query error (barn): {err}")
        if _barn_cache["data"] is not None:
            resp = jsonify(_barn_cache["data"])
            resp.headers['Cache-Control'] = 'public, max-age=60'
            return resp
...
@app.route('/api/klubb/events')
def get_klubb_events():
    now = time.time()
    if _klubb_cache["data"] is not None and now - _klubb_cache["ts"] < CACHE_TTL:
        resp = jsonify(_klubb_cache["data"])
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp
    conn = get_db_connection()
    if not conn:
        if _klubb_cache["data"] is not None:
            resp = jsonify(_klubb_cache["data"])
            resp.headers['Cache-Control'] = 'public, max-age=60'
            return resp
        return jsonify({"error": "Database connection failed"}), 500
    try:
        ...
        cursor.execute("""
            SELECT id, title, date, end_date, venue, category, link, featured, highlight, crystal_color
            FROM events
            WHERE date >= NOW() AND category = 'Klubb'
            ORDER BY date ASC
        """)
        events = fmt_events(cursor.fetchall())
        cursor.close()
        conn.close()
        _klubb_cache["data"] = events
        _klubb_cache["ts"] = now
        resp = jsonify(events)
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp
    except mysql.connector.Error as err:
        print(f"Database query error (klubb): {err}")
        if _klubb_cache["data"] is not None:
            resp = jsonify(_klubb_cache["data"])
            resp.headers['Cache-Control'] = 'public, max-age=60'
            return resp
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>https://kristall.info/</loc>
    <lastmod>{today}</lastmod>
    <changefreq>daily</changefreq>
    <priority>1.0</priority>
  </url>
  <url>
    <loc>https://kristall.info/om</loc>
    <lastmod>2026-04-09</lastmod>
    <changefreq>monthly</changefreq>
    <priority>0.5</priority>
  </url>
  <url>
    <loc>https://kristall.info/anmal</loc>
...
def om():
    if request.host.startswith('barn.'):
        return send_from_directory('static/barn', 'om.html')
    if request.host.startswith('klubb.'):
        return send_from_directory('static/klubb', 'om.html')
...
BOK_FILE   = os.path.join(_APP_DIR, 'bokmassan_state.txt')
...
def get_wow():
    return jsonify({'mode': read_wow_mode()})
...
@app.route('/api/bokmassan', methods=['GET'])
def get_bok():
    return jsonify({'mode': read_bok_mode()})

@app.route('/api/admin/bokmassan', methods=['POST'])
def set_bok():
    data = request.get_json() or {}
    mode = data.get('mode', 'off')
    if mode not in ('off', 'on'):
        return jsonify({'error': 'Invalid mode'}), 400
    with open(BOK_FILE, 'w') as f:
        f.write(mode)
    return jsonify({'mode': mode})

@app.route('/api/bokmassan/events')
def get_bok_events():
    now = time.time()
    if _bok_cache["data"] is not None and now - _bok_cache["ts"] < CACHE_TTL:
        resp = jsonify(_bok_cache["data"])
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp
    conn = get_db_connection()
    if not conn:
        if _bok_cache["data"] is not None:
            return jsonify(_bok_cache["data"])
        return jsonify({"error": "Database connection failed"}), 500
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("""
            SELECT id, title, date, venue, category, link, featured, highlight, crystal_color
            FROM events
            WHERE date >= NOW() AND source = %s
            ORDER BY date ASC
        """, (BOKMASSAN_SOURCE,))
        events = fmt_events(cursor.fetchall())
        cursor.close()
        conn.close()
        _bok_cache["data"] = events
        _bok_cache["ts"] = now
        resp = jsonify(events)
        resp.headers['Cache-Control'] = 'public, max-age=300'
        return resp
    except mysql.connector.Error as err:
        print(f"Database query error (bokmassan): {err}")
        if _bok_cache["data"] is not None:
            return jsonify(_bok_cache["data"])
        return jsonify({"error": "Database query failed"}), 500
...
@app.route('/wow')
def wow_page():
    # Samma sida som startsidan – ingen egen mall. app.js läser pathname och
    # filtrerar till enbart Way Out West-kategorin (se filters.js: wowOnly).
    #
    # WoW-läget måste vara aktiverat i /admin. Är det av finns ingen giltig
    # festivalvy – skicka till vanliga kalendern. 302, aldrig 301: läget slås
    # om och en permanent redirect skulle cachas av webbläsaren.
    if read_wow_mode() != 'on':
...
@app.route('/bokmassan')
def bokmassan_page():
    # Samma mönster som /wow: startsidans HTML, app.js läser pathname och
    # hämtar /api/bokmassan/events i stället för stadens flöde. Vyerna blandas
    # aldrig. 302, aldrig 301 – läget slås av efter mässan.
    if read_bok_mode() != 'on':
        return redirect('/', code=302)
    return send_from_directory('static', 'index.html')
